...
def main():
    ...
    # PS gains
    # reference_angles = default.KINEMATICS_DATA['joints_displ_amp']

    min_activation_deg = 10

    n_joints_bend     = default.N_JOINTS_AXIS - 2
    reference_angles  = np.ones(default.N_JOINTS_AXIS)
    reference_angles *= np.deg2rad(min_activation_deg) / n_joints_bend

    ps_gains = default.get_scaled_ps_gains(
