...
def get_step_current(t_start, t_end, unit_time, amplitude, append_zero=True):
    ...
    Args:
        ...
        Without that trailing 0, Brian reads out the last value in the array (=amplitude)
        for all indices > t_end.
...
def get_sinusoidal_current(t_start, t_end, unit_time,
    ...
    if t_end > t_start:  # if deltaT is zero, we return a zero current
        ...
        sincurr = np.sin(phi)
        sincurr = (direct_current + sincurr * amplitude)
        tmp[t_start: t_end + 1, 0] = sincurr
    curr = b2.TimedArray(tmp, dt=1. * unit_time)
    return curr

def get_sweep_current(t_start, t_end, unit_time,
                           amplitude, frequency_start, frequency_end, direct_current,
                           phase_offset=0.0, append_zero=True):
    """Creates a sweep current. If t_start == t_end, then ALL entries are 0.

    Args:
        t_start (int): start of the sweep wave
        t_end (int): end of the sweep wave
        unit_time (Quantity, Time): unit of t_start and t_end. e.g. 0.1*brian2.ms
        amplitude (Quantity, Current): maximum amplitude of the signal e.g. 3.5*brian2.uamp
        frequency_start (Quantity, Hz): Start Frequency of the sweep. e.g. 0.5*brian2.kHz
        frequency_end   (Quantity, Hz): End Frequency of the sweep. e.g. 0.5*brian2.kHz
        direct_current(Quantity, Current): DC-component (=offset) of the current
        phase_offset (float, Optional): phase at t_start. Default = 0.
        append_zero (bool, optional): if true, 0Amp is appended at t_end+1. Without that
            trailing 0, Brian reads out the last value in the array for all indices > t_end.
    ...
    assert b2.units.fundamentalunits.have_same_dimensions(direct_current, b2.amp), \
        "direct_current must have the dimension of current. e.g. brian2.uamp"
    assert b2.units.fundamentalunits.have_same_dimensions(frequency_start, b2.Hz), \
        "frequency_start must have the dimension of 1/Time. e.g. brian2.Hz"
    assert b2.units.fundamentalunits.have_same_dimensions(frequency_end, b2.Hz), \
        "frequency_end must have the dimension of 1/Time. e.g. brian2.Hz"
    ...
    if t_end > t_start:  # if deltaT is zero, we return a zero current
        ...
        frequency_sweep = frequency_start * unit_time + (frequency_end - frequency_start) * (time_steps - t_start * unit_time) / (t_end - t_start)
        phase = 2 * math.pi * np.cumsum(frequency_sweep) + phase_offset

        sincurr = np.sin(phase)
        sincurr = (direct_current + sincurr * amplitude)
        tmp[t_start: t_end + 1, 0] = sincurr
    curr = b2.TimedArray(tmp, dt=1. * unit_time)
    return curr

def get_noise_current(t_start, t_end, unit_time, mean, sigma, seed_value=None, append_zero=True):
    """
    Creates a noisy current. If t_start == t_end, then ALL entries are 0.

    Args:
        t_start (int): start of the sine wave
        t_end (int): end of the sine wave
        unit_time (Quantity, Time): unit of t_start and t_end. e.g. 0.1*brian2.ms
        sigma (Quantity, Current): std of the noise e.g. 3.5*brian2.uamp
        append_zero (bool, optional): if true, 0Amp is appended at t_end+1. Without that
            trailing 0, Brian reads out the last value in the array for all indices > t_end.
    ...
    assert isinstance(t_start, int), "t_start_ms must be of type int"
    assert isinstance(t_end, int), "t_end must be of type int"
    assert b2.units.fundamentalunits.have_same_dimensions(sigma, b2.amp), \
        "sigma must have the dimension of current. e.g. brian2.uamp"
    ...
    tmp = np.zeros((tmp_size, 1)) * b2.amp
    if t_end > t_start:  # if deltaT is zero, we return a zero current
        noisevals = mean * np.ones( tmp_size ) + sigma * np.random.randn( tmp_size )
        tmp[t_start: t_start + tmp_size, 0] = noisevals
...
def get_spikes_current(t_spikes, unit_time, amplitude, append_zero=True):
    ...
    All values in each column are 0 except one, the spike time as specified in t_spikes
    is set to amplitude.
...
def sum_currents(current_1, current_2, unit_time):
    '''
    Creates a unique timed array out of two timed arrays
    '''
    assert current_1.values.shape == current_2.values.shape, "TimedArrays must have the same shape"
    current_values = ( current_1.values + current_2.values ) * b2.amp
    return  b2.TimedArray( current_values, dt=1. * unit_time)

def stack_currents(currents, unit_time):
    '''
    Creates a unique timed array out of a list of timed arrays

    Args:
    currents (list) : list of timed arrays

    Returns:
    TimedArray: Brian2.TimedArray
    '''
    current_values = np.array( [ curr.values[:,0] for curr in currents ] ) * b2.amp
    return  b2.TimedArray( current_values.T, dt=1. * unit_time)
...
def plot_sinusoidal_current_example():
    ...
    current = get_sinusoidal_current(
        ...
        b2.us,
        amplitude=2 * b2.uamp,
        frequency=1.5 * b2.kHz,
        direct_current=1.5 * b2.uamp,
        phase_offset=math.pi / 6,
        append_zero=False)

    data = current.values[:, 0] / b2.uamp
    plt.plot(data, lw=3)
    plt.title("get_sinusoidal_current(100, 1100, b2.us, amplitude=2*b2.uamp,\nfrequency=1.5*b2.kHz,"
